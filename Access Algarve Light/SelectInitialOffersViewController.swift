//
//  SelectInitialOffersViewController.swift
//  Access Algarve Light
//
//  Created by Daniel Santos on 21/03/2018.
...
class SelectInitialOffersViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, CLLocationManagerDelegate {
    ...
    var selectedOffers: [Int]! = []
    var totalpossiblesavings: Double = 0
    
    @IBOutlet var outletsTableView: UITableView!
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        DispatchQueue.main.async() {
            ...
            if self.selectedOffers?.count != nil {
                if self.selectedOffers.contains(indexPath.row) {
                    cell.voucherSelected.isOn = true
                }
            }
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let viewSavingsViewController = segue.destination as? ViewSavingsViewController else {return}
        viewSavingsViewController.savingsamount = totalpossiblesavings
    }
    
    @IBAction func voucherSwitchChanged(_ sender: UISwitch) {
        guard let cell = sender.superview?.superview as? ViewControllerTableViewCell else {return}
        if let indexPath = outletsTableView?.indexPath(for: cell) {
            var curSavings:Double = 0
            for offer in outlets[indexPath.row].offers {curSavings += Double(offer.max_savings)!}
            if sender.isOn {
                selectedOffers.append(indexPath.row)
                totalpossiblesavings += curSavings
            } else {
                for (index, selectedoffer) in selectedOffers.enumerated() {
                    if selectedoffer == indexPath.row {
                        selectedOffers.remove(at: index)
                        totalpossiblesavings -= curSavings
                    }
                }
            }
            totalSavingsLabel.text = String(totalpossiblesavings) + "€"
        }
    }
